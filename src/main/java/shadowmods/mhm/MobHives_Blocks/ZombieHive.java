...
import shadowmods.mhm.MobHives_Entities.EntityHiveZombieRegular;
import shadowmods.mhm.MobHives_ImportantClasses.mhm_Blocks;

public class ZombieHive extends Block {
  public IIcon[] icons = new IIcon[6];
  
  public ZombieHive(Material material) {
    super(material);
    setBlockName("ZombieHive");
    setBlockTextureName("mhm:zh");
  }
  ...
  public void updateTick(World world, int x, int y, int z, Random rndm) {
    ...
    if (world.getClosestVulnerablePlayer(x, y, z, 25.0D) != null) {
      ...
      for (int v = 0; v < Config.ssp; v++)
        world.spawnEntityInWorld(new EntityHiveZombieRegular(world, x, (y + 1), z));
    } 
  }
}
